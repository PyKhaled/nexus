  ...
  module Data
    ...
    def deep_merge(left, right, path = [])
      ...
      right.each do |key, value|
        string_key = key.to_s
        if result.key?(string_key) && result[string_key].is_a?(Hash) && value.is_a?(Hash)
          result[string_key] = deep_merge(result[string_key], value, path + [string_key])
        elsif result.key?(string_key) && path.last == "services"
  ...
  class Compiler
    ...
    def write(result, output_directory, force: false)
      ...
      if File.exist?(destination) && Dir.children(destination).any? && !force
        raise Error, "output directory is not empty: #{destination}; pass --force to replace generated files"
      end

      FileUtils.mkdir_p(destination)
      compose = compose_for_output(result.compose, result.selection, destination)
      files = {
        "compose.yml" => Data.yaml(compose),
        "selection.yaml" => Data.yaml(result.selection),
      ...
      files.each { |name, content| atomic_write(File.join(destination, name), content) }
      result.runtime_files.each do |name, content|
        atomic_write(File.join(destination, "runtime", name), content)
      end
      copy_production_assets(destination) if result.selection.dig("deployment", "environment") == "production"
      destination
    end
    ...
    def plan(selection_or_path)
      result = compile(selection_or_path)
      {
    ...
    def validate_selection(selection)
      ...
      raise ValidationError, "missing required selection field #{e.key.inspect}"
    end
    ...
    def apply_deployment_policy!(compose, selection, components, context)
      ...
      if production
        prepare_production_mounts!(compose)
        prepare_production_gateway!(compose)
      end
    ...
    def render_environment_example(selection, components, artifacts, context)
      ...
      lines = [
        "# Generated by nexus-compose #{VERSION}; values are examples, never secrets.",
    ...
    def lock_file(selection, components, artifacts)
      selection_json = JSON.generate(Data.canonical(selection))
      {
        "apiVersion" => "nexus.io/composition-lock/v1alpha1",
        "generator" => {"name" => "nexus-compose", "version" => VERSION},
        "selectionDigest" => "sha256:#{Digest::SHA256.hexdigest(selection_json)}",
    ...
    def render_readme(selection, components, policy_report)
      ...
      <<~MARKDOWN
        ...
        This directory was generated by `nexus-compose #{VERSION}`. Do not edit
        generated files directly; change the selection or catalog and regenerate.
    ...
    def prepare_production_mounts!(compose)
      keycloak = compose.dig("services", "keycloak")
      if keycloak
      ...
      database = compose.dig("services", "keycloak-db")
      if database
        ...
        compose["volumes"] ||= {}
        compose["volumes"]["keycloak-db-data"] ||= {}
      end
    end

    def prepare_production_gateway!(compose)
      gateway = compose.dig("services", "gateway")
      return unless gateway

      gateway["environment"] ||= {}
      gateway["environment"]["GATEWAY_TLS_REDIRECT"] = "1"
      gateway["environment"]["TLS_CERTIFICATE"] = "/etc/nginx/ssl/fullchain.pem"
      gateway["environment"]["TLS_CERTIFICATE_KEY"] = "/etc/nginx/ssl/privkey.pem"
      gateway["ports"] = Array(gateway["ports"])
      gateway["ports"] << "${GATEWAY_HTTPS_PORT:-443}:443"
      gateway["ports"] = gateway["ports"].uniq
      gateway["volumes"] = Array(gateway["volumes"])
      gateway["volumes"] << "${TLS_CERT_DIR:?TLS_CERT_DIR is required}:/etc/nginx/ssl:ro"
      gateway["volumes"] << "./assets/gateway/templates-tls/01-default-tls.conf.template:/etc/nginx/templates/01-default-tls.conf.template:ro"
      if compose.dig("services", "website")
        gateway["volumes"] << "./assets/gateway/templates-tls/11-app-tls.conf.template:/etc/nginx/templates/11-app-tls.conf.template:ro"
      end
      if compose.dig("services", "keycloak")
        gateway["volumes"] << "./assets/gateway/templates-tls/21-auth-tls.conf.template:/etc/nginx/templates/21-auth-tls.conf.template:ro"
      end
      if compose.dig("services", "status")
        gateway["volumes"] << "./assets/gateway/templates-tls/25-status-tls.conf.template:/etc/nginx/templates/25-status-tls.conf.template:ro"
        compose.dig("services", "status", "environment")["ORIGIN"] = "https://${STATUS_DOMAIN:?STATUS_DOMAIN is required}"
      end
    end
    ...
    def validate_package(directory)
      ...
      raise ValidationError, "generated package is missing: #{missing.join(', ')}" unless missing.empty?

      selection = Data.load_yaml(File.join(directory, "selection.yaml"))
      ...
      expected_digest = "sha256:#{Digest::SHA256.hexdigest(JSON.generate(Data.canonical(selection)))}"
      unless lock["selectionDigest"] == expected_digest
        raise ValidationError, "selection digest does not match compose.lock.yaml"
      end
      unless policy["status"] == "passed"
        raise PolicyError, "generated package policy status is #{policy['status'].inspect}"
      end
      ...
      {"status" => "passed", "selectionDigest" => expected_digest}
